...
export async function POST(request:NextRequest){try{const user=await requireUser();if(user.role!=='admin')throw new Error('FORBIDDEN');await enforceMutation(request,user.email);const body=await request.json().catch(()=>({})),count=await rpc<number>('retry_sync_jobs',{p_email:user.email,p_ids:Array.isArray(body.ids)?body.ids:null});safeLog('sync_jobs_retried',{count});return NextResponse.json({ok:true,count})}catch(error){return apiError(error)}}
