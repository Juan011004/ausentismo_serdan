...
type Job={id:number;record_id:string};type Row={id:string;regional:string;gerencia:string;cedula:string;nombre:string;cargo:string;source_position:string;fecha_ingreso:string|null;operational_date:string;reported_value:string}
export async function POST(request:NextRequest){
 ...
 const jobs=await rpc<Job[]>('take_sync_jobs',{p_limit:100});if(!jobs.length)return NextResponse.json({claimed:0,synced:0,failed:0})
 ...
 try{
  ...
  await Promise.all(jobs.map(j=>rpc('finish_sync_job',{p_id:j.id,p_ok:true,p_error:null})));safeLog('sheet_sync_finished',{claimed:jobs.length,synced:jobs.length,failed:0});return NextResponse.json({claimed:jobs.length,synced:jobs.length,failed:0})
 }catch(error){const message=error instanceof Error?error.message:'UNKNOWN';await Promise.all(jobs.map(j=>rpc('finish_sync_job',{p_id:j.id,p_ok:false,p_error:message})));safeLog('sheet_sync_finished',{claimed:jobs.length,synced:0,failed:jobs.length,code:message.split(':')[0]});return NextResponse.json({claimed:jobs.length,synced:0,failed:jobs.length},{status:500})}
}
